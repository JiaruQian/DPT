...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def init_model(self):
        ...
        if self._is_actor:
            ...
            # Get parallel_thinking_mode from rollout.agent config if available
            parallel_thinking_mode = self.config.rollout.agent.get('parallel_thinking_mode', 'unseen') if hasattr(self.config, 'rollout') and hasattr(self.config.rollout, 'agent') else 'unseen'
            
            self.actor = DataParallelPPOActor(
                config=self.config.actor, 
                actor_module=self.actor_module_fsdp, 
                actor_optimizer=self.actor_optimizer,
                parallel_thinking_mode=parallel_thinking_mode
            )
