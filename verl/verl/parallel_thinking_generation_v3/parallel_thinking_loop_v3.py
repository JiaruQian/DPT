...
class ParallelThinkingAgentLoopV3(AgentLoopBase):
    ...
    def init_class(cls, config, tokenizer, **kwargs):
        ...
        # Add parallel_thinking_mode configuration: "unseen" or "seen"
        # "unseen": parallel paths have independent position encoding and attention isolation (default)
        # "seen": parallel paths share position encoding and no attention isolation (ablation study)
        cls.parallel_thinking_mode = config.actor_rollout_ref.rollout.agent.get('parallel_thinking_mode', 'unseen')
        print(f"Parallel thinking mode: {cls.parallel_thinking_mode}")
        assert cls.parallel_thinking_mode in ['unseen', 'seen'], f"Invalid parallel_thinking_mode: {cls.parallel_thinking_mode}, must be 'unseen' or 'seen'"
    ...
    async def run(
        ...
        def append_tokens(new_ids: list[int],
            ...
            # For "seen" mode, skip all position and attention mask adjustments
            if self.parallel_thinking_mode == 'seen':
                print(f"[SEEN MODE] Skipping position and attention mask adjustments for parallel paths")
                return

            # Original "unseen" mode logic below
